...
description: "列出 Claude Code 项目会话，按序号或 sessionId 选择后，经二次确认删除 .jsonl 与同名附件目录。"
metadata:
  version: "1.2.0"
---
...
4. **复述删除清单**：列出将删除的 `.jsonl` 与同名子目录，要求用户回复 `确认删除`。
5. **二次确认后删除**：只在收到强确认后删除。
...
- 如果能从 `$CLAUDE_SESSION_ID` 获取当前会话 id，删除清单中必须剔除该 id；如果无法获取，不要声称已自动识别当前会话。
...
- **当前会话被选中**：如能识别当前 sessionId，自动剔除并说明原因。
